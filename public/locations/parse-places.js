...
const getLocations = async () => {
  mapList.forEach(async (map) => {
    axios
      .get(
        `https://stats.wogames.info/json/locations-markers.json?location=${map.id}`,
      )
      .then((result) => {
        const path = `./locations/${map.dir}.json`;

        fs.writeFile(path, JSON.stringify(result.data, null, 2), (error) => {
          if (error) {
