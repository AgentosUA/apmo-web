...
const MapOverlay = observer<{
  ...
  const onMissionUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const islandClassName = getIslandClassNameByPboFile(e.target.files?.[0]).toLowerCase();
  ...
  useEffect(() => {
    if (missionEntity?.data?.briefing?.diary?.length) {
      const diaryIds = missionEntity?.data?.briefing.diary.map((item) => item.id);
  ...
  return (
    <>
      ...
      />
      <Overlay.Header
        title={planEntity.title || missionEntity?.data?.missionName || mapsEntity.selectedMap.name}
        onBack={() => {
        ...
        rightCorner={
          ...
          <div
            className="max-w-40 bg-neutral-700 truncate text-sm text-white/50 cursor-pointer px-2 border border-white/20 border-solid rounded-md flex items-center gap-2 hover:bg-a3-orange"
            onClick={onSharePlan}
          >
            <span className='truncate'>{window.location.href}</span> <MdCopyAll className='shrink-0' />
          </div>
        </div>
            <DateClock
      ...
      <Overlay.MenuWrapper>
        <Overlay.Menu>
          <Overlay.MenuItem isActive={active.map} onClick={() => onMenuItemClick('map')}>
          ...
          <Overlay.MenuItem isActive={active.markers} onClick={() => onMenuItemClick('markers')}>
            <Localize translationKey="widgets:mapOverlay:markers" />
          </Overlay.MenuItem>

          <Overlay.MenuItem isActive={active.mission} onClick={() => onMenuItemClick('mission')}>
          ...
          <View.Condition if={Boolean(missionEntity?.data?.fileName)}>
            <Overlay.MenuItem isActive={active.plan} onClick={() => onMenuItemClick('plan')}>
        ...
        <View.Condition if={active.mission}>
          <Overlay.Menu variant="secondary">
            ...
            <View.Condition if={Boolean(missionEntity?.data?.fileName)}>
              ...
              {missionEntity?.data?.groups.some((item) => item.side === 'West') && (
                <Overlay.MenuItem
              ...
              {missionEntity?.data?.groups.some((item) => item.side === 'East') && (
                <Overlay.MenuItem
              ...
              {missionEntity?.data?.groups.some((item) => item.side === 'Independent') && (
                <Overlay.MenuItem
                  isActive={active.slotsIndependent}
                  onClick={() => onMenuItemClick('mission', 'slotsIndependent')}
                >
        ...
        <View.Condition
          if={Boolean(missionEntity?.data?.briefing?.diary?.some((key) => active[key.id]))}
        >
          ...
          <Overlay.Menu variant="secondary">
            <Overlay.MenuItem>
              <ul className={styles.list}>
                {missionEntity?.data?.dlcs?.map((dlc) => <li key={dlc}>{dlc}</li>)}
              </ul>
            </Overlay.MenuItem>
          </Overlay.Menu>
        </View.Condition>
        ...
        <View.Condition if={active.slotsBluefor}>
          <Overlay.Menu variant="secondary">
            <Overlay.MenuItem>
              <MissionSlotList groups={missionEntity?.data?.groups} side="West" />
            </Overlay.MenuItem>
          </Overlay.Menu>
        </View.Condition>
        ...
        <View.Condition if={active.slotsOpfor}>
          <Overlay.Menu variant="secondary">
            <Overlay.MenuItem>
              <MissionSlotList groups={missionEntity?.data?.groups} side="East" />
            </Overlay.MenuItem>
          </Overlay.Menu>
        </View.Condition>
        ...
        <View.Condition if={active.slotsIndependent}>
          <Overlay.Menu variant="secondary">
            <Overlay.MenuItem>
              <MissionSlotList groups={missionEntity?.data?.groups} side="Independent" />
            </Overlay.MenuItem>
          </Overlay.Menu>
        </View.Condition>
            ...
            <Overlay.MenuItem>
              <ul className={styles.list}>
                {missionEntity?.data?.briefing?.intel?.day && (
                  <li>
                    <b>Date:</b> {dayjs(missionEntity?.data?.briefing?.intel?.day).format('DD')},{' '}
                    {dayjs(missionEntity?.data?.briefing?.intel?.month).format('MMMM')}
                  </li>
                )}
                <li>
                  <b>Time:</b> {dayjs(missionEntity?.data?.briefing?.intel?.hour).format('hh')}:
                  {dayjs(missionEntity?.data?.briefing?.intel?.minute).format('mm')}
                </li>
                <li>
                  <b>Description:</b> {missionEntity?.data?.briefing?.intel?.overviewText}
                </li>
              </ul>
            </Overlay.MenuItem>
          </Overlay.Menu>
